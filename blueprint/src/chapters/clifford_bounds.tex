...
\begin{proof}
  ...
  The intended route is Ross--Selinger together with
  Kliuchnikov--Maslov--Mosca: solve the one-qubit $R_z$ approximation problem
  with logarithmic $T$-count, synthesize the resulting exact one-qubit unitary
  over $\mathbb{D}[\omega] = \mathbb{Z}[\tfrac1{\sqrt2}, i]$, and translate that
  circuit into the $\{H,T\}$ alphabet with linear overhead.  That engine is
  partially formalized in \texttt{RossSelinger/} and \texttt{KMM/}, which are
  currently paused: the sources are in the tree but are not registered as
  libraries in \texttt{lakefile.toml}, are not built, and are outside the scope
  of this blueprint.  Resuming that work is the natural way to close this
  lemma.
\end{proof}
