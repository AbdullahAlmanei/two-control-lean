...
\begin{definition}
  ...
  together with embedded $H$, $S$, $S^\dagger$, and $R_z(\theta)$.  It belongs
  to the older decomposition route of Section~\ref{sec:easy_gate_layer} and is
  not used by the main theorem.
\end{definition}

\begin{lemma}
  \label{lem:cliffordrz_easy}
  \lean{TwoControl.Clifford.Universal.CliffordRzGate.easyGate}
  \leanok
  Every paper gate is an easy gate.
\end{lemma}

\begin{proof}
  \leanok
  \uses{def:gate_cliffordrz, def:gate_easy}
  Case analysis on the five clauses; $C(X)$ is a particular unitary two-qubit
  gate and the four one-qubit clauses match literally.
\end{proof}
