...
\rightline{Clifford+T is Universal; 100 minutes; July 1, 2026} 
...
they are rotations by irrational angles about orthogonal axes.
Orthogonality allows us to apply the Euler-style decomposition stated in
Lemma~\ref{lem:generalized-euler-decomposition} below.
Irrationality implies that the integer powers of $G_1$ and $G_2$ can come 
arbitrarily close to any rotation about their respective axes, 
as expressed by Lemma~\ref{%
lem:approximate-g-to-the-m-when-is-a-rotation-by-an-irr-multiple-of-pi} 
below.

Lemma~\ref{lem:properties-of-g1-g2} summarizes the key properties of $G_1,G_2$.

\begin{lemma}
\label{lem:properties-of-g1-g2}
$\det(G_1) = \det(G_2) = 1$ and
$Tr(G_1) = Tr(G_2) = \sqrt{1 + 1/\sqrt{2}}$ and 
$G_1G_2 \neq G_2G_1$.
\end{lemma}
...
% \left(
% \begin{array}{cc}
%  2 + \sqrt{2} - \sqrt{2} i        &  -6 + \sqrt{2} - (4 + \sqrt{2}) i \\
%  6 - \sqrt{2} - (4 + \sqrt{2}) i  &  2 + \sqrt{2} + \sqrt{2} i
% \end{array}
% \right)
% \\
% G_2G_1 &=&
...
% \right)
% \)
% and we see that the off-diagonal real parts change sign
% so $G_1G_2 \neq G_2G_1$.
\end{proof}

We chose $G_1$ and $G_2$ such that
they have determinant 1, 
are rotations by irrational angles about orthogonal axes, and 
do not commute. 
These are exactly the properties needed for the lemmas that follow.
...
&&
A_2^2=I,
\label{eq:a1-squared-equals-a2-squared-equals-i}
\\
G_1 = e^{i\alpha_1 A_1}
&&
G_2 = e^{i\alpha_2 A_2}
...
$\alpha_1,\alpha_2$.
We will not need explicit formulas for them, but we will rely on
two key properties that are expressed by
Lemma~\ref{lem:a1-and-a2-anticommute} and
Lemma~\ref{lem:trace-of-g1-and-trace-of-g2-expressed-via-alphas}.

\newpage

\begin{lemma}
\label{lem:a1-and-a2-anticommute}
$A_1 A_2 = - A_2 A_1$.
\end{lemma}
...
% but Lemma~\ref{lem:properties-of-g1-g2} says that $G_1G_2 \neq G_2G_1$.
% \end{proof}
...
a generalized Euler decomposition of a unitary matrix $U$
with $\det(U) = 1$ for two orthogonal rotation axes $A_1,A_2$
that satisfy $A_1 A_2 = - A_2 A_1$
(Lemma~\ref{lem:a1-and-a2-anticommute}).
...
If the real numbers $a,b,c$ were integers, we would be done:
...
The idea is that irrational rotations have integer powers that 
are dense among all rotations about the same axis.
...
Then $e^{i\lambda\pi}$ is a root of unity, so it is an algebraic integer.
Therefore,
$e^{-i\lambda\pi}$ is also an algebraic integer.
Let $x = e^{i\lambda\pi} + e^{-i\lambda\pi}$.
Since algebraic integers are closed under addition, we get:
\[
x 
\;\;=\;\;
...
x^2-1 &=& \frac{1}{\sqrt{2}}.
\)
since algebraic integers are closed under subtraction and multiplication, we get
that $\frac{1}{\sqrt{2}}$ is an algebraic integer.
However, $\frac{1}{\sqrt{2}}$ has minimal polynomial
\[
2X^2-1,
\]
which is not monic, so $\frac{1}{\sqrt{2}}$ is not an algebraic integer.
This is a contradiction so $\lambda$ is irrational.
\end{proof}
...
From Equation~\ref{eq:a1-squared-equals-a2-squared-equals-i}, 
we get
\(
e^{i 2 \pi A_1} = I 
&&
...
If the integers $m,n,p$ were nonnegative, we would be done:
...
We have seen how the proof of Lemma~\ref{lem:approximation-of-rz}
